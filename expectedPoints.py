...
def simulate_game(transition_matrix, points_per_state, num_possessions=70):
...
num_simulations = 100
for team in teams:
    team_name, fg_pct_element, fg2_element, fg2a_element, fg2_pct_element, fg3_element, fg3a_element, fg3_pct_element, ft_element, fta_element, ft_pct_element, orb_element, drb_element, trb_element, ast_element, stl_element, blk_element, tov_element, pf_element, pts_element = data(team)
    possessions = tov_element + fg2a_element + fg3a_element + ft_element // 2.5
    ...
    predicted_scores = [simulate_game(transition_matrix, points_per_state) for _ in range(num_simulations)]
    expected_points = np.mean(predicted_scores)
    print(f'Predicted Score for {team}: {expected_points:.2f} points')
    percent_error = (abs(pts_element / 26  - expected_points) / expected_points) * 100
